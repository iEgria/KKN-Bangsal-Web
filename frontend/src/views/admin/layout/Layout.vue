        ...
        <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow bg-white m-3 border-radius-xl" navbar-scroll="true">
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 text-truncate">
                        <li class="breadcrumb-item text-sm text-capitalize text-truncate" v-for=" (path, index) in this.$route.name.split('.')" v-bind:key="index">{{ path.replace('-', ' ') }}</li>
                    </ol>
                    <h6 class="fw-bolder text-capitalize mb-0">{{ this.$route.name.split('.')[this.$route.name.split('.').length - 1].replace('-', ' ') }}</h6>
                </nav>
